// Winner logic for Calendle
import { DateModel } from '../models/DateModel';
import { SHAPES, ShapeNames } from '../lib/common';

export function validateWinner(board) {
    const b = board;
...
export function findWinner(placedShapes, remainingShapes, board) {
    if (placedShapes.length === ShapeNames.length && remainingShapes.length === 0) {
        return validateWinner(board);
    }
    return false;
}
